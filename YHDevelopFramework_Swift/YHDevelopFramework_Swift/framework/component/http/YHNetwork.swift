...
class YHNetwork: YHLoggerProtocol {
    ...
    private var result: DataResponse<Any>?
    ...
    func getRequest(url: String, parameter: Parameters? = nil) -> YHNetwork {
        ...
        Alamofire.request(url, method: .get, parameters: parameter, headers: header).validate().responseJSON(queue: utilityQueue) { (response) in
    ...
    func postRequest(url: String, parameter: Parameters? = nil ) -> YHNetwork {
        ...
        Alamofire.request(url, method: .post, parameters: parameter, encoding: JSONEncoding.default, headers: header).validate().responseJSON { (response) in
            switch response.result {
            case .success:
                self.log.info("** network success:\n\(response.request!)\n\(response.timeline)\n\(response.response)\n\(response.result.value)")
                self.mainQueue.async {
